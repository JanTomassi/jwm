	...
	if ((c = wintoclient(ev->window))) {
		if (ev->value_mask & CWBorderWidth)
			...
			if ((c->x + c->w) > m->mx + m->mw && c->isfloating)
				c->x = m->mx +
				       (m->mw / 2 - WIDTH(c) / 2); /* center
                                                                               in x
                                                                               direction
                                                                             */
			if ((c->y + c->h) > m->my + m->mh && c->isfloating)
				c->y = m->my +
				       (m->mh / 2 - HEIGHT(c) / 2); /* center
                                                                                in y
                                                                                direction
                                                                              */
...
void drawbar(Monitor *m)
{
	...
	w = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeSel]);
	x = drw_text(drw, x, 0, w - lrpad / 2, bh, lrpad / 4, m->ltsymbol, 0);
...
void focusmon(const Arg *arg)
{
	...
	int pointer_query = XQueryPointer(dpy, selmon->barwin, &root_return,
	...
	if (pointer_query) {
		...
		XWarpPointer(dpy, None, m->barwin, 0, 0, 0, 0, m->mw / 2,
			     m->mh / 2);
	}
	selmon = m;
	focus(NULL);
